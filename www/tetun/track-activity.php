...
/* ================= Completion Function ================= */
function updateCompletion($conn, $activityId, $category, $activity) {
    ...
    stripos($category, "- Lian") !== false && $activity === "Soe letra ha'u") {

        $completedText = "bele soletra liafuan simples sira";
    ...
    stripos($category, "- Lian") !== false && $activity === "Soe letra") {
        ...
        $completedText = " bele soletra liafuan simples";
    ...
    stripos($category, "- Sobu monta") !== false && $activity === "Vizita avo feto") {
        ...
        $completedText = "bele ordena parte sira husi istória (Visita Avó feto)";
    ...
    stripos($category, "- Númeru") !== false && $activity === "Dezeñu númeru 1 to'o 10") {
        ...
        $completedText = "bele buka tuir númeru sira";
    ...
    elseif (stripos($category, "Transporte") !== false &&
    stripos($category, "- Istoría") !== false && $activity === "Viajen ba eskola") {

        $completedText = 'kompleta ona istória "Viajen ba eskola"';

    }
    else {
    error_log("NO MATCH FOUND → category: $category | activity: $activity");
    $completedText = "Tentadu";
}
    ...
    mysqli_stmt_bind_param($upd, "si", $completedText, $activityId);
...
mysqli_stmt_bind_param($stmt,"is",$pid,$sid);
...
// if ($statusFlag === 'completed' && !$row) {
//     echo json_encode([
//         "status"=>"error",
//         "message"=>"No row to complete"
//     ]);
//     exit;
// }
...
if ($updateTarget === 'activity_name' && $link !== '') {
     $refetch = mysqli_prepare($conn,
        "SELECT id, pid, sid, category1, activity_name
         FROM activity_result
         WHERE id = ?"
    );
...
// Prepare
$chk = mysqli_prepare($conn, $sql);

// Bind dynamically
mysqli_stmt_bind_param($chk, $types, ...$params);
...
if ($existing = mysqli_fetch_assoc($chkRes)) {
    ...
    error_log("Duplicate found → deleted row");
...
//     // ✅ Switch to existing row
//     $activityId = $existingId;

//     // ✅ Just refetch — NO completion here
//     $refetch = mysqli_prepare($conn,
//         "SELECT id, pid, sid, category1, activity_name
//          FROM activity_result
//          WHERE id = ?"
...
// }
   ...
   // 4️⃣ Update activity_name on the correct row
    if($link === 'Fim de semana nian' || $link === 'Konta bola' || $link === 'Mantein bee-dalan sira moos'){
    $activityResult ='';
    if ($link === 'Fim de semana nian') {
    $activityResult = 'bele identifika atividades fins de semana';
}else if($link === 'Konta bola'){
    $activityResult = "bele sekuénsia númeru 1 to'o 3 (Sura bola sira)";
}
// else if($link === 'Konta liman-fuan'){
//     $activityResult = 'aprende kona-ba konta liman-fuan';
// }
else {
    $activityResult = 'bele identifika no soe foer (Tasi-ibun moos)';
}
...
// 🔥 ALWAYS refresh latest row before using it
$refetch = mysqli_prepare($conn,
    "SELECT id, pid, sid, category1, activity_name
     FROM activity_result
     WHERE id = ?"
